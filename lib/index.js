...
// var projectParser = require('./gradle-project-parser');
...
function inspect(root, targetFile, options) {
  ...
  return getAllDeps(root, targetFile, options)
    .then((res) => {
      console.log(res);
      return pkg;
    })
...
function getAllDeps(root, targetFile, options) {
  ...
  console.log('**** getAllDeps args', args);
  return subProcess.execute(
    getCommand(root, targetFile),
    args,
    {cwd: root})
    // Go to parser + become a tree
    .then(res => console.log('***** res', res)); //parse the projects out + parse the tree out
}
...
function buildArgs(root, targetFile, gradleArgs, subProject) {
  ...
  // } else {
  args.push('printSolvedDepsTreeInJson');
  //}
  args.push('-q');
  if (targetFile) {
